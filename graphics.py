...
from parameters import WIDTH, HEIGHT, WHITE, GRAY, LIGHT_BLUE, BLACK
from ui import ShapeSelectionWindow # Импортируем класс из ui.py
    ...
    for face, z_avg, color in faces_with_depth:
        ...
        if len(face_points) >= 3 and not are_points_collinear(face_points):
            try:
                ...
                print(f"Warning: Skipping invalid polygon with points: {face_points}")
        ...
        visible_face_count += 1 # Увеличиваем счётчик отрисованных (и видимых) граней
    return visible_face_count

def are_points_collinear(points, tolerance=1e-5):
    """Проверяет, лежат ли 3 или более точки на одной прямой."""
    if len(points) < 3:
        return False
    ...
    for i in range(2, len(points)):
        ...
        if area > tolerance:
             return False
    return True # Все точки лежат на одной прямой
...
def render_ui(screen, current_shape, camera_distance, rotation_mode, back_face_culling, visible_face_count,
    ...
    """Рендерит интерфейс пользователя (кнопки, ползунки, текст)."""
    # Отрисовка кнопок
...
def render_shape_selection_window(screen, event, selection_window):
    """
    Отрисовывает окно выбора фигуры и обрабатывает события.
    Возвращает:
    - str: имя выбранной фигуры, если нажата OK.
    - False: если нажата Cancel или окно закрыто.
    - None: если окно все еще открыто и ожидает действий.
    """
    result = selection_window.handle_event(event)
    selection_window.draw(screen)

    if result is not None: # OK или Cancel нажаты
        return result
    if event.type == pygame.QUIT: # Обработка закрытия окна Pygame
        return False # Или можно вернуть None и обработать QUIT отдельно
    return None # Окно все еще открыто
